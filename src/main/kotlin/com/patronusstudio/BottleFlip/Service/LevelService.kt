...
class LevelService {
    ...
    fun updateLevelDetail(levelsModel: LevelsModel) :BaseResponse{
        val levelUpdateSql =
            "UPDATE levels SET  level = ${levelsModel.level},star = ${levelsModel.star} where id = ${levelsModel.id}"
        val levelUpdateResult = sqlRepo.setData(levelUpdateSql)
        if(levelUpdateResult is BaseSealed.Error){
            return ErrorResponse(message = "Level güncellenirken bir hata oluştu.",HttpStatus.NOT_ACCEPTABLE)
        }
        return SuccesResponse(status = HttpStatus.OK, data = "Level güncellendi")
    }
}